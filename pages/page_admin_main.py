...
class PageAdminMain(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tk.Label(title_frame, text=f"디스플레이 크기: {utils.get_display_size()}", font=(setting.DEFAULT_FONT, 16), fg="black").pack()
        tk.Label(title_frame, text=f"PID: {utils.get_program_pid()}", font=(setting.DEFAULT_FONT, 16), fg="black").pack()
        self.uptime = utils.get_now_datetime() - setting.START_TIME
        self.uptime_label = tk.Label(title_frame, text=f"작동시간: {self.uptime}", font=(setting.DEFAULT_FONT, 16), fg="black")
        def update_uptime():
        ...
        tk.Button(self.admin_frame, text="문 열어놓기", font=(setting.DEFAULT_FONT, 16, 'bold'), height=2, width=56, command=lambda: self.controller.show_page("PageAdminForceOpen")).pack()
