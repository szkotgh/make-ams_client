...
class PageAdminTestNFC(tk.Frame):
    def __init__(self, parent, controller):
        super().__init__(parent)
        self.controller = controller

        # 상태 변수
        self.detect_nfc_value = None
    ...
    def _detect_nfc(self, _nfc_result: str):
        print(f"[PageAdminTestNFC] NFC detected: {_nfc_result}")
        
        if self.controller.now_page != "PageAdminTestNFC" or self.test_running:
            print(f"[PageAdminTestNFC] NFC ignored - current page: {self.controller.now_page}, test running: {self.test_running}")
            return

        with self._test_lock:
            if self.test_running:
                return
            self.detect_nfc_value = _nfc_result
            self.test_running = True
            print(f"[PageAdminTestNFC] NFC test started, transitioning to PageAdminTestNFC")
            self.controller.show_page("PageAdminTestNFC")
            threading.Thread(target=self._run_test_flow, daemon=True).start()

    def _check_nfc_status(self):
        while True:
            if self.test_running:
                time.sleep(1)
                continue
            ...
            if hardware_manager.nfc.is_initialized == False:
                ...
                self._set_sub_title("NFC를 인식시켜주세요")
            time.sleep(1)
            continue
    ...
    def _run_test_flow(self):
        ...
        self._set_sub_title(f"{self.detect_nfc_value}")
        time.sleep(3)
        self._set_sub_title("NFC를 인식시켜주세요")
        with self._test_lock:
            self.test_running = False
