        ...
        while True:
            try:
                ...
                if not self._device:
                    ...
                    import auth_manager, config
                    auth_manager.service.qr_status_hw = config.STATUS_DISABLE
                    time.sleep(1)
                    continue

                print("QR Listener Initialized Successfully")
                import auth_manager, config
                auth_manager.service.qr_status_hw = config.STATUS_ENABLE
                keymap = {
        ...
        while True:
            ...
            if need_reinit:
                ...
                try:
                    ...
                    import auth_manager, config
                    auth_manager.service.nfc_status_hw = config.STATUS_ENABLE
                except Exception as e:
                    print(f"NFC Initialize failed: {e}")
                    self.nfc_initialized = False
                    import auth_manager, config
                    auth_manager.service.nfc_status_hw = config.STATUS_DISABLE
